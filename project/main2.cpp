...
void initialize()
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, positionBuffer);
	// Send the vertex position data to the current buffer
	glBufferData(GL_ARRAY_BUFFER, labhelper::array_length(positions1) * sizeof(float), positions1,
		GL_STATIC_DRAW);
	...
	glBindBuffer(GL_ARRAY_BUFFER, colorBuffer);
	// Send the vertex color data to the current buffer
	glBufferData(GL_ARRAY_BUFFER, labhelper::array_length(colors1) * sizeof(float), colors1, GL_STATIC_DRAW); //color buffer for t1?
